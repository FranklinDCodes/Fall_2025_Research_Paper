...
\section{Introduction}
    ...
    It can be thought of as a classification problem, where the goal is to classify "good" and "bad" times to invest.
    In that sense, it is more important in trading to have high levels of precision than it is to be highly accurate.
    Missing 100 possible opportunities is worth it for the chance to be certain that a price will go up 1 time.
    This need for precision and more than accuracy is something that may be given consideration with classification models, but goes completely unquantified with a regression model.

    This work aims to be able to quantify that need with a regression model.
    If a price prediction model is combined with a simple algorithm for deciding when to trade, then as the prediction model becomes more conservative with it's predictions, the algorithms outcomes should increase in precision.
    This paper will explore a method by which to produce that result, and examine its effect on profitability.
